...
export function StudentDetailsModal({ student, loading, error, appointments, history, tab, onTab, onClose, onEmergency, emergencyOpen }) {
  return <Modal wide title="Student record" onClose={onClose} topmost={!emergencyOpen}>
    ...
    {loading ? <p role="status" className="animate-pulse p-8 text-sm text-gray-500">Loading record…</p> : student && <>
      ...
      </div><footer className="flex flex-wrap items-center justify-between gap-3 border-t border-gray-200 bg-gray-50 px-5 py-4 dark:border-gray-700 dark:bg-gray-900/30"><p className="text-xs text-gray-500">Clinic records and encounter history</p><button onClick={onEmergency} className={primary}><Plus size={16} />Add Emergency Encounter</button></footer>
    </>}
  </Modal>;
}
