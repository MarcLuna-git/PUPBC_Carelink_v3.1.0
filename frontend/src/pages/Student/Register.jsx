...
import { Eye, EyeOff, Loader2, Hash, Mail, Phone, Lock, Calendar, CheckCircle, ShieldCheck } from 'lucide-react';
...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (otpStep) {
      ...
      setMessage('');
      try {
        const res = await authService.verifyRegistration(form.email.trim(), otp);
        if (res.success) {
          setMessage('Email verified and account created! Redirecting to login...');
    ...
    try {
      ...
      if (res.success) {
        setOtpStep(true);
        setMessage('A 6-digit verification code was sent to your email. Enter it below to finish creating your account.');
      } else {
    ...
    <div className="min-h-screen relative overflow-hidden flex items-center justify-center py-8 px-4">
      ...
      <div className="relative z-10 w-full max-w-xl">
        <div className="flex flex-col items-center mb-5">
          <img src={clinicLogo} alt="PUPBC CareLink logo" className="w-20 h-20 object-cover drop-shadow-lg rounded-full" />
        </div>
        <div className="bg-white rounded-3xl shadow-2xl p-6 sm:p-8">
          ...
          {message && (
            <div className={`mb-4 p-3 rounded-xl text-sm text-center ${message.toLowerCase().includes('sent') || message.toLowerCase().includes('created') ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>
              {message}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="rounded-2xl border border-yellow-200 bg-yellow-50 p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <p className="font-semibold text-maroon-900 text-sm">Verify your email</p>
                    <p className="text-xs text-gray-600 mt-1">We sent a 6-digit code to <strong>{form.email}</strong>. The code expires in 5 minutes.</p>
                    <input
            ...
            <button type="submit" disabled={loading}
              ...
              {loading ? <><Loader2 className="w-4 h-4 animate-spin" /><span>{otpStep ? 'Verifying email...' : 'Sending verification code...'}</span></> : <span>{otpStep ? 'Verify & Create Account' : 'Sign Up'}</span>}
            </button>
          </form>

          <p className="text-center text-sm text-gray-500 mt-5">
            Already have an account? <Link to="/login" className="text-maroon-800 font-semibold hover:underline">Log in</Link>
          </p>
        </div>
