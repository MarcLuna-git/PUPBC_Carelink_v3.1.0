...
const Appointments = () => {
  ...
  const [pageLoading, setPageLoading] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem('carelink.student.appointments') || '[]').length === 0;
    } catch {
      return true;
    }
  });
  ...
  const [appointments, setAppointments] = useState(() => {
    try {
      const cached = JSON.parse(localStorage.getItem('carelink.student.appointments') || '[]');
  ...
  const fetchAppointments = useCallback(async () => {
    try {
      ...
      if (response.data.success) {
        ...
        localStorage.setItem('carelink.student.appointments', JSON.stringify(normalized));
      }
  ...
  const handleSubmit = async (e) => {
    ...
    const selectedSlot = new Date(`${form.date} ${form.time}`);
    if (Number.isNaN(selectedSlot.getTime()) || selectedSlot < new Date()) {
      setMessageType('error');
      setMessage('Please choose a present or future appointment time.');
      setTimeout(() => setMessage(''), 3000);
      return;
    }
    ...
    try {
      ...
      let response;
      if (editingId) {
        response = await api.put(`/student/appointments/${editingId}`, payload, { headers: { Authorization: `Bearer ${token}` } });
      ...
      setMessage(
        ...
        (err.response ? `Failed to save appointment (HTTP ${err.response.status}).` : 'Cannot connect to the appointment service.')
      );
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div className="relative"><Calendar className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                    <input type="date" name="date" value={form.date} onChange={handleChange} required min={getLocalDateString()} className="w-full border border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-2xl pl-10 pr-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-maroon-500" />
                  </div>
                </div>
                <div>
                  ...
                  <div className="relative"><Clock className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                    <select name="time" value={form.time} onChange={handleChange} required className="w-full border border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-2xl pl-10 pr-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-maroon-500 appearance-none">
                      ...
                      {timeSlots.map(t => {
                        ...
                        const isPast = form.date && new Date(`${form.date} ${t}`) < new Date();
                        const unavailable = slot.isFull || isPast;
                        return (
                          <option key={t} value={t} disabled={unavailable}>
                            {t} {form.date ? (isPast ? '(Past)' : unavailable ? '(Full)' : `(${slot.available}/10 slots)`) : ''}
                          </option>
                        );
                      })}
                    </select>
